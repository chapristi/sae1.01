...
def calculatePoints(attempts: int, max_attempts: int, point_win:int) -> int:
    """
...
def game(currentPlayers: CurrentPlayers, conn: Connection)->None:
    ...
    while not gameRiddle.isOver:
        ...
        if int(choice) == gameRiddle.numberToGuess:
            ...
            winner = currentPlayers.player1
            gameRiddle.isOver = True
    winningInformationsInit(winningInformations, gameRiddle.colName,0,calculPoints(gameRiddle.attempts,gameRiddle.maxAttempts,gameRiddle.pointWin),gameRiddle.pointLoose,False)
